...
public class WindowSensor {
	public static void main(String args[]) {
		...
		boolean CurrentWindow = false;// Windows state: false == 정상, true == 비정상
		...
		if (em != null) {
			...
			while (!Done) {
				...
				// 이벤트 메니저에게 이벤트 보내기
				PostWindowStatus(em, CurrentWindow);

				mw.WriteMessage(" WindowSatus::  " + CurrentWindow + "not break!");
	...
	/***************************************************************************
	 * CONCRETE METHOD:: PostWindowStatus Purpose: This method posts the specified
	...
	static private void PostWindowStatus(EventManagerInterface ei, boolean windowbreak) {
		// Here we create the event.

		Event evt = new Event((int) 3, String.valueOf(windowbreak));

		// Here we send the event to the event manager.

		try {
			ei.SendEvent(evt);// System.out.println( "Sent Window Event" );
		} // try
		catch (Exception e) {
