...
public class FireSensor {
	public static void main(String args[]) {
		...
		boolean CurrentStatus = false;// fire state: false == 정상, true == 비정상
		...
		if (em != null){
			...
			*********************************************************************/
			...
			while ( !Done ) {
				...
				if (sprinklerState){ //스프링클러가 작동하지않는 경우
					CurrentStatus = true;//불남
				} 
				if (!sprinklerState){ //스프링클러가 작동하는 경우
					CurrentStatus = false;//불안남
				}
				try	{
					Thread.sleep( Delay );
