...
public class MotionDetectionSensor {
	public static void main(String args[]) {
		...
		boolean CurrentMotion = false;// Windows state: false == 정상, true == 비정상
		...
		if (em != null) {
			...
			mw.WriteMessage("   Initial Window Set:: " + CurrentMotion);
			/********************************************************************
			...
			while (!Done) {
				...
				// 이벤트 메니저에게 이벤트 보내기
				PostMotionDetectionStatus(em, CurrentMotion);

				mw.WriteMessage(" DoorSatus::  " + CurrentMotion + "not break!");
	...
	private static void PostMotionDetectionStatus(EventManagerInterface ei, boolean currentMotion) {
		// Here we create the event.

		Event evt = new Event((int) 5, String.valueOf(currentMotion));

		// Here we send the event to the event manager.

		try {
			ei.SendEvent(evt);// System.out.println( "Sent MotionDetection Event" );
		} // try
		catch (Exception e) {
